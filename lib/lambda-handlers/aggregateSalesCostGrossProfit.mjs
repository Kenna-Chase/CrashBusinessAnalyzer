...
export const handler = async (event) => {
    ...
    if (!event.body) {
        return {
            statusCode: 400,
            body: JSON.stringify({ event: event, error: 'body with yearMonth and companyName is required.' }),
        };
    }

    // Expect these attributes in the event body payload
...
function aggregateSalesCostData (data) {
    ...
    return { labor: {sales: laborTotals.sales, cost: laborTotals.cost},
        ...
        overallTotals: { salesTotal: salesTotal, costTotal: costTotal, gpTotal: salesTotal - costTotal } ,
        otherAdditionsAndDeductions: {additions: totalAdditions, deductions: totalDeductions} };
}
